const jobs = [
  ...
  function searchJobs(titleQuery, locationQuery) {
    ...
    jobs.forEach(job => {
      ...
      if (jobTitleLower.includes(titleQueryLower) && jobLocationLower.includes(locationQueryLower)) {
        result.result.push(job);
        result.count++;
      }
    });
  ...
  document.getElementById('search-button').addEventListener('click', function() {
    const titleQuery = document.getElementById('job-title').value;
    const locationQuery = document.getElementById('job-location').value;
